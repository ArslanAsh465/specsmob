...
@section('content')
    ...
    <div class="shadow-sm rounded bg-white p-3 mt-4">
        <form action="{{ route('backend.reviews.update', $review->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <select name="mobile_id" id="mobile_id" class="form-select @error('mobile_id') is-invalid @enderror" required>
                        ...
                        @foreach($mobiles as $mobile)
                            <option value="{{ $mobile->id }}" {{ old('mobile_id', $review->mobile_id) == $mobile->id ? 'selected' : '' }}>{{ $mobile->name }}</option>
                        @endforeach
                    </select>
                    @error('mobile_id')
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $review->title) }}" placeholder="Enter review title" required>
                    @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="seo_title" id="seo_title" class="form-control @error('seo_title') is-invalid @enderror" value="{{ old('seo_title', $review->seo_title) }}" placeholder="Enter SEO Title">
                    @error('seo_title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-12">
                    ...
                    <input type="text" name="seo_keywords" id="seo_keywords" class="form-control @error('seo_keywords') is-invalid @enderror" value="{{ old('seo_keywords', $review->seo_keywords) }}" placeholder="Enter SEO Keywords">
                    @error('seo_keywords')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-12">
                    ...
                    <textarea name="seo_description" id="seo_description" rows="2" class="form-control @error('seo_description') is-invalid @enderror">{{ old('seo_description', $review->seo_description) }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <select name="status" id="status" class="form-control">
                        ...
                        <option value="archived" {{ old('status', $review->status) == 'archived' ? 'selected' : '' }}>Archived</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
