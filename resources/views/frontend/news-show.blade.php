...
@section('content')
    @php
        ...
        $image = $newsItem->image ? asset('storage/' . $newsItem->image) : $defaultImage;
    @endphp

    <!-- Top Image -->
    <div class="mb-4">
        <img src="{{ $image }}" alt="{{ $newsItem->title }}" class="img-fluid w-100 rounded shadow-sm" style="max-height: 450px; object-fit: cover;">
    </div>
    ...
    <div class="card border-0 shadow-sm rounded overflow-hidden mb-4">
        ...
        <div class="card-header bg-light">
            ...
            <small class="d-flex justify-content-between mt-1 text-muted">
                <span>
                    <i class="bi bi-calendar3 me-1"></i> {{ $newsItem->created_at->diffForHumans() }}
                </span>
                <span>
                    <i class="bi bi-chat-left-text me-1"></i> {{ $newsItem->comments->count() }} Comments
                </span>
            </small>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded overflow-hidden mb-4">
        <div class="card-header bg-light">
            <h5 class="fw-bold mb-0">
                <i class="bi bi-chat-left-text me-1"></i> Comments ({{ $newsItem->comments->count() }})
            </h5>
        </div>
